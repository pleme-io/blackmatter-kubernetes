# k3s 1.30.14+k3s2 version pins
#
# Shared component versions come from lib/versions/kubernetes-1.30.nix.
# Hashes: use `nix build .#k3s-1-30` and fix hashes from error output.
let
  shared = import ../../../lib/versions/kubernetes-1.30.nix;
in {
  ...
  imagesVersions = builtins.fromJSON (builtins.readFile ./1_30-images.json);
}
